...
for space in param_space:
    ...
    for n in range(args.n_samples):
        ...
        # ESN cell
        esn_wv, esn_c3 = create_esn_models(use_cuda)
        ...
        for k in range(10):
            ...
            for i, data in enumerate(sfgram_loader_test_wv):
                ...
                # Predict
                y_predicted = esn_wv(inputs)

                # Append
                wv_outputs[i] = y_predicted
            ...
            for i, data in enumerate(sfgram_loader_test_c3):
                ...
                # WV predicted
                wv_predicted = wv_outputs[i]
                wv_length = wv_predicted.size(1)

                # Sample rate
                """sample_rate = int(math.floor(c3_length / wv_length))
                print(wv_length)
                print(c3_length)
                print(sample_rate)
                # Subsample y_predicted to wv_predicted
                y_subsampled = torch.mean(y_predicted.unfold(1, sample_rate, sample_rate), dim=3)"""

                # Downsample character predictions
                y_subsampled = torch.from_numpy(signal.resample(y_predicted.numpy(), wv_length, axis=1)).type(torch.FloatTensor)

                # Average
                wv_c3_predicted = (wv_predicted + y_subsampled) / 2.0

                # Show
                """plt.plot(wv_predicted[0].numpy(), 'b')
                plt.plot(y_subsampled[0].numpy(), 'y')
                plt.plot(wv_c3_predicted[0].numpy(), 'r')
                plt.show()"""

                # Add to y and ^y
                if i == 0:
                    total_predicted = wv_c3_predicted
                else:
                    total_predicted = torch.cat((total_predicted, wv_c3_predicted), dim=1)
                # end if
