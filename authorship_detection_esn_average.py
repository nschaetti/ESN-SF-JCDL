...
for space in param_space:
    ...
    for n in range(args.n_samples):
        ...
        for k in range(10):
            ...
            # Truth vector
            truth_tensor = torch.zeros(len(sfgram_loader_test))

            # Prediction for each threshold
            n_threshold = 100
            thresholds = torch.linspace(0.0, 1.0, n_threshold)
            prediction_tensor = torch.zeros(n_threshold, len(sfgram_loader_test))
            ...
            for j, threshold in enumerate(thresholds):
                # Above threshold => 1.0
                predicted_labels = prediction_tensor[j] == 1.0

                try:
                    tp_fp = float(torch.sum(predicted_labels))
                    tp_fn = float(torch.sum(truth_tensor))
                    tp = float(torch.sum(truth_tensor[predicted_labels]))
            ...
            # Print
            print(truth_tensor)
            print(prediction_tensor[torch.argmax(f1_scores)])
